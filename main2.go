...
func main() {
	...
	log.SetOutput(file)
	args := initParams()
	fmt.Println("cron:", args.Style)
    // 创建一个新的 cron 实例
    c := cron.New()
    fmt.Println("任务添加时间:", time.Now().Format("2006-01-02 15:04:05"))
    // 添加一个每分钟执行一次的任务
    _, err = c.AddFunc(args.Style, func() {
    ...
    // 启动 cron 实例
    c.Start()
    // 为了保持程序运行，我们可以添加一个阻塞操作
    // 例如，在这里我们使用一个简单的 sleep 操作
    // 注意：在实际生产环境中，您可能希望使用更优雅的机制来保持程序运行
    select {}
}
...
func initParams() Args {
	...
	flag.StringVar(&args.Style, "s", "0/10 * * * *", "@every 1m（每分钟），@hourly（每小时），@daily（每天），@weekly（每周），@monthly（每月），以及自定义的 cron 表达式（如 0/30 * * * * 表示每30分钟执行）")
	//@every 1m（每分钟），@hourly（每小时），@daily（每天），@weekly（每周），@monthly（每月），以及自定义的 cron 表达式（如 0/30 * * * * 分别表示分钟、小时、日、月、星期）
	flag.Parse()
	return args
}
